package com.example.integradoraprueba.controllers.mecanico;
...
@WebServlet(name = "mecanico", urlPatterns = {
        "/mecanico/main",
        "/mecanico/orders-view",
        "/mecanico/save-order",
        "/mecanico/save-service",
        "/mecanico/update-status",
        "/mecanico/update-statusend",
        "/mecanico/all-products"

})
public class ServletMecanico extends HttpServlet {

    private String action;
    private String redirect = "/mecanico/main";
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        switch (action) {
            case "/mecanico/main":
                redirect = "/views/mecanico/mecanico-main.jsp";
                break;
            case "/mecanico/orders-view":
                ...
                redirect = "/views/mecanico/orders-view.jsp";
                break;
            case "/mecanico/all-products":
                req.setAttribute("products", new DaoProducts().findAllProducts());
                redirect ="/views/mecanico/products-view.jsp";
                break;
        }
        ...
        switch (action) {
            case "/mecanico/save-order":
                try {
                    ...
                    if (result) {
                        redirect = "/mecanico/orders-view?&result=true&message=" + URLEncoder.encode("¡Éxito! Orden almacenada correctamente.",
                                StandardCharsets.UTF_8);
                    } else {
                        redirect = "/mecanico/orders-view?&result=false&message=" + URLEncoder.encode("¡Error! Orden no almacenada correctamente.",
                                StandardCharsets.UTF_8);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            case "/mecanico/save-service":
                try {
                    ...
                    if (result) {
                        redirect = "/mecanico/orders-view?result=true&message=" + URLEncoder
                                .encode("Servicio registrado correctamente", StandardCharsets.UTF_8);
                    } else {
                        redirect = "/mecanico/orders-view?result=false&message=" + URLEncoder
            ...
            case "/mecanico/update-status":
                try {
                    ...
                    if (result) {
                        redirect = "/mecanico/orders-view?result=true&message=" + URLEncoder.encode("Orden finalizada correctamente.", StandardCharsets.UTF_8);
                    } else {
                        redirect = "/mecanico/orders-view?result=false&message=" + URLEncoder.encode("Error al finalizar la orden.", StandardCharsets.UTF_8);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    redirect = "/mecanico/orders-view?result=false&message=" + URLEncoder.encode("Error al finalizar la orden: " + e.getMessage(), StandardCharsets.UTF_8);
                }
                break;
            case "/mecanico/update-statusend":
                try {
                    String orderIdStr = req.getParameter("id_order");
                    long orderId = Long.parseLong(orderIdStr);
                    System.out.println(orderId);
